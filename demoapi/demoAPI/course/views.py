...
class GetAllCourseAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not mydata.is_valid():
            return Response('sai data roi', status=status.HTTP_400_BAD_REQUEST)
        title = mydata.data['title']
        content = mydata.data['content']    
        price = mydata.data['price']
        dt = Course.objects.create(title=title, price=price, content=content)
        return Response(data=dt.id, status=status.HTTP_200_OK)